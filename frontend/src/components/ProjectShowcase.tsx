...
function ProjectCard3D({
  ...
  return (
    <div
      ...
      className={`${gameMode ? "project-card-3d group" : ""} relative rounded-xl border border-zinc-800 bg-zinc-900/50 overflow-hidden`}
    >
      {gameMode && (
        <div
    ...
    <section id="projects">
      <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <p className="text-sm text-zinc-500">
          {gameMode ? "Tilt, sweep, and inspect the build vault." : "Selected builds and shipped work."}
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {projects.map((project, i) => (
          <ProjectCard3D key={project.id} project={project} index={i} gameMode={gameMode} />
        ))}
      </div>
    </section>
  );
